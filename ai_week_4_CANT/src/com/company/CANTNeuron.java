...
//   change it to private later
  ...
  public CANTNeuron(int neuronId ,CANTNet net) {
     ...
     double temp = Main.random.nextFloat()*100;
...
//undone put this generic stuff somewhere else.
  ...
  //when using compensatory learning based on the post-synaptic neuron
...
//--------------------stuff for running the net-------------
  ...
  public void resetActivation(){
    ...
    public boolean spontaneouslyActivate(){
      ...
      if (Main.random.nextFloat() <= 0.03) return true;
      return false;
    }
...
////-----------------Stuff for the learning rules---------------------
    ...
    //How much to reduce an association based on Strength of the Connection
...
//      double modifiedStrength = currentStrength/(parentNet.getAxonalStrengthMedian()*2);
    ...
    public double getJustDecreaseBase(double currentStrength) {
        double modifiedStrength = currentStrength/(parentNet.getAxonalStrengthMedian()*2);
        return (modifiedStrength);
      }

    public double getJustIncreaseBase(double currentStrength) {
      double modifiedStrength = currentStrength/(parentNet.getAxonalStrengthMedian()*2);
      double estimateNeuronNotFires = (1-modifiedStrength);
	  ...
	  for (int synap=0; synap < getCurrentSynapses(); synap++) {
	    ...
	    if (toNeuron.getFired()) {
	      if (!isInhibitory){
	        ...
	        modification = getJustIncreaseBase(connectionStrength);
	        modification *= toCompensatoryStrengthModifier;
		  ...
		  //System.out.println("Inc Exc "+this.getId()+" "+toNeuron.getId()+" "+synapses[synap].getWeight()+" "+modification + " "+fromCompensatoryStrengthModifier + " " + totalConnectionStrength + " " + parentNet.getSaturationBase());
		        ...
		        modification = getJustIncreaseBase(1+connectionStrength);
		        modification  *= parentNet.getLearningRate();
		        connectionStrength = (connectionStrength)+modification;
		        synapses[synap].setWeight(connectionStrength);
/*untested compensatory mechanism
	    ...
	    //if to Neuron is inactive
	      ...
	      if (!isInhibitory){
if (parentNet.getName().compareTo("HddenNet") == 0) 	    	  
System.out.println("Dec Exc "+this.getId()+" "+toNeuron.getId()+" "+synapses[synap].getWeight());
	        totalToConnectionStrength = toNeuron.getIncomingStrength();
	        toCompensatoryWeakModifier = 
	          getWeakCompensatoryModifier(totalToConnectionStrength,
		    toNeuron.parentNet.getSaturationBase());
	        modification = getJustDecreaseBase(connectionStrength);
	        modification *= toCompensatoryWeakModifier;
	        if (modification > 1.0) modification = 1.0;
	        modification  *= parentNet.getLearningRate();
	        connectionStrength = connectionStrength-modification;
	        if (connectionStrength < 0.001) connectionStrength = 0.001;
	        synapses[synap].setWeight(connectionStrength);
if (parentNet.getName().compareTo("HddenNet") == 0) {	    	  
System.out.println("         " + modification + " " + totalConnectionStrength + " " + toCompensatoryWeakModifier);
}
		  //System.out.println("dec Exc "+this.getId()+" "+toNeuron.getId()+" "+synapses[synap].getWeight()+" "+modification + " " + fromCompensatoryWeakModifier);
	          ...
	          modification = getJustDecreaseBase(1+connectionStrength);
	          modification  *= parentNet.getLearningRate();
	          connectionStrength = connectionStrength-modification;
	          synapses[synap].setWeight(connectionStrength);
	/* untested compensatory mechanism
