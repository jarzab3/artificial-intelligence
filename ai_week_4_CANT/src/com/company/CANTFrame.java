...
public void actionPerformed (ActionEvent evt) {
  if (evt.getSource()==startButton)
    ...
    parentNet.runAllOneStep(Main.CANTStep);
	Main.CANTStep++;
    //Main.runOneStep(Main.CANTStep);
  }
     ...
     //reset run does not reset the weights of synapses abd the currentpattern
          ...
          Main.CANTStep=0;
          matrix.clear();
          timeStepLabel.setText("TStep:  " + Main.CANTStep);
          outputLabel.setText("Active Neurons: " );
          repaint();
   }
  else if (evt.getSource()== setDelay) {
    userParamField.setText(Integer.toString(Main.delayBetweenSteps));
    userParamType=23;
  }
...
//         NewCols = ColSize;
     ...
     if ((measureCycle>= 1) && ( measureCycle<= 1000))
      ...
      Main.delayBetweenSteps = Integer.parseInt(userParamField.getText());
    }
    ...
    public void runOneStep(int cantStep) {
        ...
        repaint();
        try{
           Thread.sleep(Main.delayBetweenSteps);
        }
